...
const ListBar = ({ closeDropDown }) => {
  return (
    <div className="flex-auto flex flex-col justify-between h-52 rounded-lg font-bold mt-2 text-[#3C3C3C]  md:text-white ">
      <ul className="md:text-sm  w-full">
        <li>
          <Link
            onClick={closeDropDown}
            to={"/dashboard"}
            href="#"
            className="block py-2  max-md:rounded px-3  hover:bg-[#489CFF] w-full  "
          >
            Beranda
          </Link>
        </li>
      ...
      <div className="md:flex md:flex-row md:justify-center">
        <button
          type="button"
          className="flex flex-row items-center space-x-2 text-xs md:text-sm md:w-32 p-2.5 focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-2 focus:ring-red-300 font-medium rounded-lg me-2 mb-2 "
        >
          <svg
